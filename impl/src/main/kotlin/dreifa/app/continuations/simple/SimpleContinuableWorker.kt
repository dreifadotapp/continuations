...
class SimpleContinuableWorker(registry: Registry) : ContinuableWorker, ContinuableWorkerThreadMonitor {
    ...
    override fun startup() {
        ...
        continuations().forEach {
            if (status(it) == ContinuationStatus.Running) {
                ...
                val input = kv.get(Key.fromUniqueId(UniqueId("$it:input")))
    ...
    override fun <O> result(id: ContinuationId): O {
        if (status(id) == ContinuationStatus.Completed) {
            val entry = kv.get(Key.fromUniqueId(UniqueId(id.id())))
            return kv.deserialise(entry)
        } else {
    ...
    override fun continuations(): ContinuationIdList {
        val pattern = LikeString("dreifa.app.continuations.events.%")
...
class WorkerThread(
    ...
    override fun run() {
        try {
            ...
            kv.put(Key.fromUniqueId(UniqueId(id.id() + ":input")), SKSValue(input, SKSValueType.Serialisable))
            ...
            kv.put(Key.fromUniqueId(UniqueId(id.id())), SKSValue(result, SKSValueType.Serialisable))
            ...
            kv.put(Key.fromString(id.id()), SKSValue(ExceptionInfo(ex), SKSValueType.Serialisable))
            ew.store(ContinuationFailedFactory.create(id, ex))
        } catch (t: Throwable) {
            kv.put(Key.fromString(id.id()), SKSValue(ExceptionInfo(t), SKSValueType.Serialisable))
